...
public class CannonEntityRenderer extends EntityRenderer<CannonEntity> {
    ...
    public void render(CannonEntity entity, float yaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        if (tick <= 4) {
            if (color != null) {
                this.fire.render(poseStack, buffer.getBuffer(RenderType.eyes(DYED_FIRE_TEXTURES[tick])), packedLight, OverlayTexture.NO_OVERLAY, color.x(), color.y(), color.z(), 1.0F);
            } else {
        ...
        tick -= 3;
        if (tick >= 0 && tick <= 4) {
            this.fire.render(poseStack, buffer.getBuffer(RenderType.entityTranslucent(SMOKE_TEXTURES[tick])), packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 0.9F - (0.07F * tick));
        }
